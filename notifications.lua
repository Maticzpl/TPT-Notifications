...
function MaticzplNotifications.DrawMenuContent()
    ...
    --Scroll Bar
    local barHeight = 12 
    if holdingScroll then
        -- Wolfram alpha saved me here xd
        notif.scrolled = - ((scrollLimit*(mouseY - 269)) / (barHeight - 141))
    end
    ...
    for i, n in ipairs(notif.notifications) do      
        ...
        if next == nil or next.title ~= title then
            if mouseX > 418 + 12 and mouseX < 418 + 193 and mouseY > lastTitleY and mouseY < y then
                if lastTitleY >= 252 then         
                    gfx.drawRect(418 + 12,lastTitleY-1,193 - 13,y - lastTitleY-1)
                    if justClicked then
                        sim.loadSave(saveID)
                    end
                end
        ...
        scrollLimit = -((y - 252) / 5 - notif.scrolled )
    end
